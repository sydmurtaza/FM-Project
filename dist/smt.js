...
class SMTGenerator {
  async verifyProgram(ssa, z3) {
    ...
    ctx.delete();
    return {
      result: result === z3.sat ? 'Satisfiable' : 'Unsatisfiable',
      examples,
      counterexamples,
      smt
    };
  ...
  async checkEquivalence(ssa1, ssa2, z3) {
    ...
    if (result === z3.unsat) {
      ...
      for (let i = 0; i < 2 && (await solver.check()) === z3.sat; i++) {
        const model = solver.model();
        counterexamples.push(this.extractModel(model, {
          statements: [...ssa1.statements, ...ssa2.statements]
        }));
    ...
    ctx.delete();
    return {
      result: result === z3.unsat ? 'Equivalent' : 'Not Equivalent',
      examples,
      counterexamples,
      smt
    };
  ...
  generateSMT(ssa, checkAssert) {
    ...
    for (const stmt of ssa.statements) {
      if (stmt.type === 'Assignment') {
        ...
        smt += `(assert (forall ((i Int)) (=> (and (>= i 0) (< i ${this.exprToSMT(stmt.rangeEnd)})) ${this.exprToSMT(stmt.condition, {
          [stmt.varName]: {
            type: 'Variable',
            name: 'i'
          }
